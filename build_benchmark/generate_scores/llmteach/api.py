...
class APIQuery:
    def __init__(
        ...
        **kwargs,
        ...
        ) and (api == "openai" or (api == "sri" and model.startswith("openai")))
        is_gemini_reasoning = (
            "gemini-3" in model and (api == "sri") or (api == "google")
        )
        if is_openai_reasoning or is_gemini_reasoning:
            logger.info("Not using system messages, setting reasoning effort.")
        ...
        if (
            api not in ["anthropic", "openai"]
            and not (api == "sri" and model.startswith("openai"))
            and batch_processing
        ):
            logger.warning(
        ...
        if reasoning_effort is not None:
            if api == "vllm":
                ...
                not self.openai_responses
                or self.batch_processing
