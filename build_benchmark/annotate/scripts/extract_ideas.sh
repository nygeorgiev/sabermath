...
if [[ $# -lt 3 ]]; then
  echo "Usage: extract_ideas.sh <parallel_count> <vllm_port> <huggingface_path>"
  exit 1
fi
...
echo "[INFO] Starting Idea Extractor on dataset $3..."
python ./ideas/extract_ideas.py $3 --api-url http://localhost:$VLLM_PORT/v1/ --statement-column problem --reasoning high
echo "[INFO] Idea Extraction finished"
